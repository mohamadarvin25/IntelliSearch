...
#     Attributes
#     ----------
#     term_id_map(IdMap): Mapping of terms to termIDs
#     doc_id_map(IdMap): Mapping of document paths to docIDs
#     data_dir(str): Path to data
#     output_dir(str): Path to output index files
#     postings_encoding: Compression strategy for postings (e.g., VBEPostings)
...
#         # To store names of all intermediate inverted indices
#         self.intermediate_indices = []

#     def save(self):
#         """Save doc_id_map and term_id_map to the output directory using pickle."""
#         # Ensure the output directory exists
#         os.makedirs(self.output_dir, exist_ok=True)

#         # Correctly construct paths without duplicating 'index'
#         terms_path = os.path.join(self.output_dir, 'terms.dict')
#         docs_path = os.path.join(self.output_dir, 'docs.dict')

#         try:
#             with open(terms_path, 'wb') as f:
#                 pickle.dump(self.term_id_map, f)
#             logger.info(f"Saved term_id_map to {terms_path}")
#         except Exception as e:
#             logger.error(f"Failed to save term_id_map: {e}")

#         try:
#             with open(docs_path, 'wb') as f:
#                 pickle.dump(self.doc_id_map, f)
#             logger.info(f"Saved doc_id_map to {docs_path}")
#         except Exception as e:
#             logger.error(f"Failed to save doc_id_map: {e}")
...
#                 self.term_id_map = pickle.load(f)
#             logger.info(f"Loaded term_id_map from {terms_path}")
#         except Exception as e:
#             logger.error(f"Failed to load term_id_map: {e}")
#             self.term_id_map = IdMap()  # Initialize empty if loading fails

#         try:
#             with open(docs_path, 'rb') as f:
#                 self.doc_id_map = pickle.load(f)
#             logger.info(f"Loaded doc_id_map from {docs_path}")
#         except Exception as e:
#             logger.error(f"Failed to load doc_id_map: {e}")
#             self.doc_id_map = IdMap()  # Initialize empty if loading fails
...
#                 self.postings_dict = merged_index.postings_dict
#                 self.average_doc_length = merged_index.average_doc_length
#             logger.info("Loaded merged inverted index successfully.")
#         except Exception as e:
#             logger.error(f"Failed to load merged inverted index: {e}")

#     def pre_processing_text(self, content):
#         """
#         Perform text preprocessing: stemming and removing stopwords.
#         """
#         stemmer = MPStemmer()
#         stemmed = stemmer.stem(content)
#         remover = StopWordRemoverFactory().create_stop_word_remover()
...
#         # Define symbols to remove using regex
#         symbols_to_remove = r'{}()[].,:;+-*/&|<>=~%?$'
#         escaped_symbols = re.escape(symbols_to_remove)
#         symbol_pattern = f'[{escaped_symbols}]'
...
#             doc_path = os.path.join(path, filename)
#             doc_id = self.doc_id_map[doc_path]
#             encoding = self.detect_encoding(doc_path)
#             try:
#                 with open(doc_path, "r", encoding=encoding, errors='replace') as f:
#                     for line in f:
#                         processed_token = self.pre_processing_text(line)
#                         tokenized_token = word_tokenize(processed_token)
#                         for token in tokenized_token:
#                             cleaned_token = re.sub(symbol_pattern, '', token)
#                             if cleaned_token:  # Ensure token is not empty after cleaning
#                                 term_id = self.term_id_map[cleaned_token]
#                                 td_pairs.append((term_id, doc_id))
#             except Exception as e:
...
#     def write_to_index(self, td_pairs, index):
#         """
#         Invert the td_pairs and store them in the index.
#         """
#         term_dict = defaultdict(dict)
...
#             logger.warning("No terms to merge.")
#             return  # No terms to merge
...
#         if len(postings_list) == 0:
#             logger.info("No postings found for the query terms.")
#             return []
#         else:
#             for posting in postings_list:
#                 df_t = len(posting[0])
#                 idf = math.log10(N / df_t) if df_t > 0 else 0
...
#     def retrieve_bm25(self, query, k=10, k1=1.2, b=0.75):
#         """
#         Perform Ranked Retrieval using the BM25 scoring scheme and the TaaT (Term-at-a-Time) framework.
#         Returns top-K retrieval results.
#         """
#         retrieval_results = []
#         list_query = []
...
#                     numerator = (k1 + 1) * tf_t_d
#                     denominator = k1 * ((1 - b) + b * (self.doc_length[posting[0][j]] / self.average_doc_length)) + tf_t_d
#                     curr_score = idf * (numerator / denominator) if denominator != 0 else 0
#                     temp_score.append((posting[0][j], curr_score))
#                 retrieval_results = merge_and_sort_posts_and_tfs(retrieval_results, temp_score)
...
#     def do_indexing(self):
#         """
#         Core indexing method using BSBI scheme with relative paths for collections.
#         """
#         logger.info(f"Indexing Data Directory: {self.data_dir}")
        
#         # Get only the final directory name if full path is provided
#         data_dir_relative = os.path.basename(self.data_dir)
        
#         try:
#             # Walk through the collections directory using relative path
#             blocks = sorted(next(os.walk(data_dir_relative))[1])
#         except StopIteration:
#             logger.error(f"No blocks found in data directory: {data_dir_relative}")
#             return
