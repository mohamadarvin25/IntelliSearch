...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        info = self.postings_dict[term]
        start_position_in_index_file = info[0]
        self.index_file.seek(start_position_in_index_file)

        # postings_list
        length_in_bytes_of_postings_list = info[2]
        postings_list_bytes = self.index_file.read(length_in_bytes_of_postings_list)
        postings_list = self.postings_encoding.decode(postings_list_bytes)

        # tf_list
        length_in_bytes_of_tf_list = info[3]
        tf_list_bytes = self.index_file.read(length_in_bytes_of_tf_list)
        tf_list = self.postings_encoding.decode_tf(tf_list_bytes)
